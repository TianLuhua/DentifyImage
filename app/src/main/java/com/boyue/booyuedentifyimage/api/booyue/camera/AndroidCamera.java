package com.boyue.booyuedentifyimage.api.booyue.camera;
...
public class AndroidCamera {
    ...
    public AndroidCamera(Context context1) {
        context = context1;
        devDisplay = ((WindowManager) context1.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
    ...
    /**
     * @param paramString 被分割的字符串
     * @return 分割后的Integer链表
     */
    protected static ArrayList<Integer> splitInt(String paramString) {
        ...
        while (localStringTokenizer.hasMoreElements()) {
            Integer localInteger = Integer.valueOf(Integer.parseInt(localStringTokenizer.nextToken()));
    ...
    /**
     * 获取前置摄像头
     * sdk 大于10摄像头（但是matorola 和 sumsung例外，api也不尽相同）
     *
     * @return 摄像头
     */
    protected Camera getFrontCamera() {
        if (fitSdkVersion()) {//sdk>=10
    ...
    protected void adjustDirection(Camera c) {
        try {
            ...
            Log.e(TAG, "adjustDirection： " + e.getMessage());
        }
    }
    ...
    protected int GetNumberOfCamera() {
        ...
        try {
            ...
            Log.e(TAG, "GetNumberOfCamera： " + e.getMessage());
        }
    ...
    protected void setDisplayOrientation(Camera camera, int degree) {
        try {
            ...
            Log.e(TAG, "setDisplayOrientation： " + e.getMessage());
        }
    }

    /**
     * 打开前置摄像头,首先判断系统是否有摄像头
     *
     * @return
     */
    protected Camera openFrontFacingCamera() {
        ...
        try {
            ...
            Log.e(TAG, "openFrontFacingCamera： " + e.getMessage());
    ...
    static class CameraInformation {
        int facing;//前后摄像头
        int orientation;//水平和垂直
        int rotation;//旋转角度
    }

    /**
     * 获取摄像头显示方向{@link #setCameraDisplayOrientation(int, Camera)}
     *
     * @param cameraId
     * @param camera
     * @return
    ...
    /**
     * 设置摄像头显示方向{@link #getCameraDisplayOrientation(int, Camera)}
     *
    ...
    /**
     * 获取与最佳匹配的尺寸，将最理想的尺寸与设备支持的尺寸做一一对比，找出最佳与本机的尺寸
     *
     * @param sizes 支持尺寸集合
     * @param w     最佳的宽
     * @param h     最佳的高
    ...
    protected Size getOptimalEqualPreviewSize(List<Size> sizes, int w, int h) {
        ...
        // Cannot find the one match the aspect ratio, ignore the requirement
        if (optimalSize == null) {
            optimalSize = getOptimalPreviewSize(sizes, PREVIEW_WIDTH, PREVIEW_HEIGHT);
        }
        return optimalSize;
    }
